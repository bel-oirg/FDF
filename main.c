...
/*   Updated: 2024/01/10 10:32:45 by bel-oirg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void	zoom_centred(t_buddha *v, char *file_name)
{
	int	x;
	int	y;
	int	diff_x;
	...
	x = get_cols(file_name);
	y = get_rows(file_name);
	v->zoom = (min_v(WIDTH / x, HEIGHT / y) * 0.7 + 1);
